...
public class SaveLoadManager {
    ...
    public void ulozHru(String fileName) throws IOException {
        try (PrintWriter out = new PrintWriter(new FileWriter(fileName))) {
            ...
            out.println(dungeonManager.getDungeon().getAktualnaMiestnost().getId());

            // ===== PREDMETY V MIESTNOSTIACH =====
            out.println("[MIESTNOSTI_PREDMETY]");
            for (Miestnost m : dungeonManager.getDungeon().getMiestnosti()) {
            ...
            // ===== POSTAVY V MIESTNOSTIACH (zdravie) =====
        ...
        try (BufferedReader in = new BufferedReader(new FileReader(fileName))) {
            ...
            while ((line = in.readLine()) != null) {
                if (line.equals("[HRAC]")) {
                    ...
                    // ak máš setter na level, použi: hrac.setLevel(level);
                    // Inventar
    ...
    private Predmet NajdiPredmetPodlaMena(String meno) {
        // ... tu doplň všetky možné predmety podľa hry ...
        switch (meno) {
            case "Gobliní tesák": return new Zbran("zbran2", "Gobliní tesák", "Ostrý zahnutý nôž", 8, 8);
            case "Silný liečivý elixír": return new Lektvar("lektvar2", "Silný liečivý elixír", "Liečivý elixír s intenzívnym účinkom", 30);
            case "Liečivý elixír": return new Lektvar("lektvar_hadanka", "Liečivý elixír", "Odmena za rozlúštenie hádanky.", 30);
    ...
    private Quest najdiQuestPodlaNazvu(String nazov) {
        for (Quest q : dungeonManager.getQuestDatabaza().getVsetkyQuesty()) {
