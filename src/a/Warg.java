...
public class Warg extends Nepriatel implements InteraktivnaPostava {
    ...
    private void zastrasitWarga(Hrac hrac) {
        if (this.pokusZastrasenie()) {
            System.out.println(getMeno() + " cúva a kňučí! Podarilo sa ti ho zastrašiť!");
            System.out.println(getMeno() + " sa stiahol a umožňuje ti bezpečný prechod.");
        } else {
            System.out.println(getMeno() + " považuje tvoj pokus o zastrašenie za výzvu a útočí s väčšou zúrivosťou!");
            int damage = new Random().nextInt(8) + 4;
            hrac.setZdravie(hrac.getZdravie() - damage);
            System.out.println(getMeno() + " ťa pohrýzol! Stratil si " + damage + " životov.");
            this.utok(hrac);
        }
    }
    ...
    private void utekZBoja(Hrac hrac, BattleSystem battleSystem) {
        ...
        if (utekHod > 15) {
            ...
            System.out.println(getMeno() + " je príliš rýchly! Dobehol ťa a skočil ti na chrbát!");
